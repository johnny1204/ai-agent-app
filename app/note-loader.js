document.addEventListener('DOMContentLoaded', async () => {
    ...
    const selfProxyUrl = '/api/note-rss';
    ...
    try {
        const response = await fetch(selfProxyUrl, { cache: 'no-store' });
        if (!response.ok) {
        ...
        const link = item.querySelector('link')?.textContent || 'https://note.com/katagaki_none';
        ...
        if (mediaThumbnail) {
            thumbnailUrl = mediaThumbnail.textContent || mediaThumbnail.getAttribute('url') || thumbnailUrl;
        }

        const pubDate = pubDateRaw
            ? new Date(pubDateRaw).toLocaleDateString('ja-JP', { year: 'numeric', month: 'long', day: 'numeric' })
            : '';
            ...
            <section class="mb-24">
                ...
                <a href="${link}" target="_blank" rel="noopener noreferrer" class="group relative block">
                    ...
                    <div class="relative bg-slate-900 border border-slate-800 rounded-xl p-6 md:p-8 hover:bg-slate-850 transition-all duration-300 flex flex-col md:flex-row gap-6 items-center">
                        <div class="flex-grow">
                            <div class="flex items-center gap-3 mb-3">
                                <span class="px-3 py-1 text-xs font-bold tracking-widest text-emerald-400 bg-emerald-950/30 rounded-full border border-emerald-900">note</span>
                                <span class="text-slate-400 text-sm">${pubDate}</span>
                            </div>
                            <h3 class="text-2xl font-bold text-slate-100 mb-3 group-hover:text-emerald-300 transition-colors line-clamp-2">${title}</h3>
                            <div class="text-slate-400 leading-relaxed text-sm line-clamp-2 mb-4">${stripHtml(description)}</div>
